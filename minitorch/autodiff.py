...
def central_difference(f: Any, *vals: Any, arg: int = 0, epsilon: float = 1e-6) -> Any:
    ...
    """
    f1 = f(*vals[:arg], vals[arg] - epsilon / 2, *vals[arg + 1:])
    f2 = f(*vals[:arg], vals[arg] + epsilon / 2, *vals[arg + 1:])
        ...
        if var.is_leaf():
            ...
            for parent, d_parent in var.chain_rule(d_var):
                if not parent.is_constant():
                    derivatives[parent.unique_id] = derivatives.get(parent.unique_id, 0.0) + d_parent
